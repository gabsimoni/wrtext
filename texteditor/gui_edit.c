...
gui_edit_init(GtkApplication *app)
{
    ...
    */
    ...
    /*
        Create the in-window top bar (toolbar-like).
        This MUST return a GtkWidget* that we can pack into vbox.
        It should include your Settings button and whatever else you need.
    */
    GtkWidget *topbar = gui_edit_menu_init(GTK_WINDOW(window), app);
    if(topbar != NULL) {
...
gui_edit_add_file(editor_file *f)
{
    ...
    *fid = last_assigned;
    ...
    /* Show widgets (works, but deprecated warnings may appear in newer GTK) */
